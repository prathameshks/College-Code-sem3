...
void display(){
	...
	for(int i = 0;i<product_rec.size();i++){
		cout<<product_rec[i].code<<" ";
		cout<<product_rec[i].name<<" ";
		cout<<product_rec[i].cost<<" ";
		cout<<product_rec[i].quantity<<endl;
	}
}
...
void search_product(){
	cout<<"Search by 1. Code 2. Name"<<endl;
	cout<<"Enter your choice:";
	cin>>sort_basis;
	item i1;
	if (sort_basis==2){
	...
	int ans = find(product_rec.begin(),product_rec.end(),i1);
	print_item(product_rec[ans]);
}
